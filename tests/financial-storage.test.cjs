...
test('old statements remain visible with their period and STALE label but cannot feed scores',async()=>{
 const now=new Date();
 records=[{ticker:'OLD',periodEnd:'2020-12-31',fetchedAt:now,payload:{version:'FINANCIAL_DATA_V1',periodEnd:'2020-12-31',periodType:'ANNUAL',currency:'USD',source:'SEC',values:{...emptyValues(),netIncome:42}}}];
 assert.equal((await loadStoredFundamentals(['OLD'])).size,0);
 const [view]=await hydrateFinancialData([{ticker:'OLD',qualityDataStatus:'COMPLETE',netIncome:999}]);
 assert.equal(view.netIncome,42);assert.equal(view.financialPeriodEnd,'2020-12-31');assert.equal(view.qualityDataStatus,'STALE');assert.equal(view.valueDataStatus,'STALE');
});
test('unverified legacy numbers cannot leak into the verified financial display',async()=>{
 records=[];
 const [view]=await hydrateFinancialData([{ticker:'NONE',netIncome:999,totalDebt:888,cashAndEquivalents:777,freeCashFlow:666,financialSource:'old',financialCurrency:'USD',financialFetchedAt:new Date()}]);
 for(const field of ['netIncome','totalDebt','cashAndEquivalents','freeCashFlow','financialSource','financialCurrency','financialFetchedAt'])assert.equal(view[field],null);
 assert.equal(view.qualityDataStatus,'UNVERIFIED');
});
test('recent reporting period with an expired acquisition is excluded from scoring',async()=>{
 const end=new Date(Date.now()-90*86400000).toISOString().slice(0,10);
 records=[{ticker:'EXPIRED',periodEnd:end,fetchedAt:new Date(Date.now()-36*86400000),payload:{version:'FINANCIAL_DATA_V1',periodEnd:end,periodType:'QUARTER',values:emptyValues()}}];
 assert.equal((await loadStoredFundamentals(['EXPIRED'])).size,0);
 assert.equal((await loadStoredFundamentals(['EXPIRED'],{includeStale:true})).get('EXPIRED').stale,true);
});
